...
void turtleQuad(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double r, double g, double b, double a) { // adds a (blit) quad to the pipeline (for better speed)
    ...
    list_append(turtools.penPos, (unitype) 67, 'h'); // blit quad signifier
    ...
    list_append(turtools.penPos, (unitype) x4, 'd');
    list_append(turtools.penPos, (unitype) r, 'd'); // duplicate colour data (wasted space)
    list_append(turtools.penPos, (unitype) g, 'd');
    list_append(turtools.penPos, (unitype) b, 'd');
    list_append(turtools.penPos, (unitype) a, 'd');
    list_append(turtools.penPos, (unitype) 67, 'h'); // blit quad signifier
    list_append(turtools.penPos, (unitype) y4, 'd');
}
    ...
    if (changed == 1) { // only redraw the screen if there have been any changes from last frame
        ...
        for (int i = 0; i < len; i += 9) {
            if (renType[i] == 'd') {
                ...
                if (i + 9 < len && renType[i + 9] == 'd' && ren[i + 7].h < 64 && (ren[i + 7].h == 4 || (fabs(ren[i].d - ren[i + 9].d) > ren[i + 2].d / 2 || fabs(ren[i + 1].d - ren[i + 10].d) > ren[i + 2].d / 2))) { // tests for next point continuity and also ensures that the next point is at sufficiently different coordinates
