...
void sliderControl(slider *selfp) { // assumes turtools.mouseX and turtools.mouseY have been set
    slider self = *selfp;
    ...
    if (turtools.mouseX > self.x - 5 && turtools.mouseX < self.x + 5 && turtools.mouseY > self.y - self.length / 2 - 5 && turtools.mouseY < self.y + self.length / 2 - self.length * 0.1 + 5 && !turtleMouseDown()) {
        self.hover = 1;
    } else {
        self.hover = 0;
    }
    if (self.mouseDown == 1) {
        double perc = (turtools.mouseY - (self.y - self.length / 2)) / ((self.y + self.length / 2 - self.length * 0.1) - (self.y - self.length / 2));
        if (perc > 1) {
            perc = 1;
        }
        if (perc < 0) {
